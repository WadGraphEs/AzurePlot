...
namespace WadGraphEs.MetricsEndpoint.Lib {
	public class MetricsConfigEndpointConfiguration : MetricsEndpointConfiguration {
		...
		public MetricsConfigEndpointConfiguration(string basedir) {
			_baseDir = basedir;
		...
		private string FindLineForConfigKeyOrNull(string configKey) {
			...
			if(!File.Exists(filepath)) {
				throw new InvalidOperationException(string.Format("Couldn't find config file: {0}", filepath));
			}
		...
		internal Microsoft.WindowsAzure.CertificateCloudCredentials GetCertificateCloudCredentials() {
			string certificateLocation = Path.Combine(_baseDir,CertificateFilename);

			if(!File.Exists(certificateLocation)) {
				throw new InvalidOperationException(string.Format("Couldn't locate certificate file in {0}", certificateLocation));
			}

            var x509Certificate = new X509Certificate2(certificateLocation, CertificatePassword, X509KeyStorageFlags.MachineKeySet);
